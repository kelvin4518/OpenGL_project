...
//The Vao & Vbo of lightbox
GLuint lightboxVAO;
GLuint lightboxVBO;
GLuint lightboxVBO_IDX;
//The Vao & Vbo of skybox
GLuint skyboxVAO;
GLuint skyboxVBO;
//vao[0] is the vao of earth
//vao[1] is the vao of planetC
//vao[2] is the vao of planetB
//vao[3] is the vao of objD
extern GLuint vao[4];
//Vao for 200 rockes.
extern GLuint objGVao[200];

extern int drawEarthSize;
extern int drawPlanetCSize;
extern int drawPlanetBSize;
extern int drawObjDSize;
extern int drawObjGSize[200];
...
float a_brightness = 2.0f;
float d_brightness = 2.0f;
float s_brightness = 2.0f;
...
void keyboard(unsigned char key, int x, int y)
{
	if (key == '\033')
		...
		if (key == 'q') {
			if (d_brightness >= 1.0f)
				d_brightness = d_brightness - 1.0f;
		}
		else {
			if (key == 'w') {
				if (d_brightness <= 30.0f) {
					d_brightness = d_brightness + 1.0f;

				}
				else {
					if (key == 'a') {
						if (a_brightness >= 1.0f) {
							a_brightness = a_brightness - 1.0f;
						}
						else {
							if (key == 'z') {
								a_brightness = a_brightness + 1.0f;
...
{
	if (right_wheel == true) {
		cameraX = (float)(1024 - x) / 1024 * 50;
		cameraY = (float)(768 - y) / 768 * 50;
	}
...
{
	if (fov >= radians(1.0f) && fov <= radians(90.0f)){
		printf("%d", wheel);
		if (wheel == 2) {
			right_wheel = true;
		}
...
GLuint loadBMP(vector<const GLchar*> faces)
{
	GLuint textureID;
...
void sendDataToOpenGL()
{
	...
	skybox.push_back("texture/sea_skybox/left.bmp");
	skybox.push_back("texture/sea_skybox/right.bmp");
	skybox.push_back("texture/sea_skybox/top.bmp");
	skybox.push_back("texture/sea_skybox/bottom.bmp");
	skybox.push_back("texture/sea_skybox/front.bmp");
	skybox.push_back("texture/sea_skybox/back.bmp");
	TextureSkybox = loadBMP(skybox);
}
...
void drawObjD(void)
{
	...
	glm::mat4 scale_M = glm::scale(glm::mat4(1.0f), glm::vec3(scale_fact));
	glm::mat4 rot_M = glm::rotate(glm::mat4(1.0f), glm::radians(45.0f), glm::vec3(0, 1, 0));
	glm::mat4 trans_M = glm::translate(glm::mat4(1.0f), glm::vec3(-1.5f, 10.0f, -5.0f));
	glm::mat4 Model = trans_M * rot_M * scale_M;
...
void drawObjG(void)
{
	...
	modelMatrices = new glm::mat4[200];
	...
	for (GLuint i = 0; i < 200; i++)
	{
		...
		GLfloat angle = (GLfloat)i / (GLfloat)200 * 360.0f;
...
void drawSkybox(void)
{
	...
	glm::mat4 projection = glm::perspective(45.0f, 4.0f/5.0f, 0.1f, 100.0f);
...
void paintGL(void)
{
	...
	// projection matrix
	common_projection = glm::perspective(fov, 1.0f, 0.1f, 200.0f);
...
void Shaders_Installer()
{
	...
	planetC_programID = installShaders("shader/Common.vs", "shader/Common.frag");
	skyboxProgramID = installShaders("shader/Skybox.vs", "shader/Skybox.frag");
	earthProgramID = installShaders("shader/Common.vs", "shader/Common.frag");
